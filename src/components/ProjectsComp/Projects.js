  ...
  render() {
    let proj = this.state.projects.map((el, ind) => {
      return (
        <li
          className={`${
            this.state.clicked === ind ? "active" : ""
          } pro-el text-primary`}
        >
          <div onMouseEnter={() => this.elementClicked(ind)}>
            {this.state.projects[ind].name}{" "}
            <a target="_blank" href={this.state.projects[ind].link}>
              <img
                className={`${
                  this.state.clicked === ind ? "active2" : ""
                } im-link`}
                src="https://cdn-icons-png.flaticon.com/512/922/922124.png"
              ></img>
            </a>
          </div>
        </li>
      ...
      <div>
        <div className="row ss">
          <div className="col-md-9">
            ...
            <img
              className="screen"
              src={`images/${this.state.projects[this.state.clicked].vid}`}
            ></img>
